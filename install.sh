...
[ -e /sbin/openrc -a -s /run/openrc/softlevel ] && VIRTUAL=false || VIRTUAL=true

if [ $(id -u) = 0 ]; then
	$VIRTUAL || { echo "This script must not run as root"; exit 1; }
	USER=root
fi
...
[ -z "${INSTALL_ARM_COMPILER+x}" ] \
	...
	&& read -r INSTALL_ARM_COMPILER \
	&& echo
	{ [ "$INSTALL_ARM_COMPILER" = 'y' -o "$INSTALL_ARM_COMPILER" = 'Y' ] && sudo apk add make gcc-arm-none-eabi newlib-arm-none-eabi; }

[ -z "${INSTALL_AVR_COMPILER+x}" ] \
	&& echo -n "Install AVR GCC? (y/N): " \
	&& read -r REPLY \
	&& echo
	{ [ "$INSTALL_AVR_COMPILER" = 'y' -o "$INSTALL_AVR_COMPILER" = 'Y' ] && sudo apk add make gcc-avr avr-libc; }

[ -z "${CAMERA_STREAMING+x}" ] \
	&& echo -n "Install Camera Streaming Packages? (y/N): " \
	&& read -r CAMERA_STREAMING \
	&& echo \
	&& { [ "$CAMERA_STREAMING" = 'y' -o "$CAMERA_STREAMING" = 'Y' ] && CAMERA_STREAMING=1; }
...
sudo addgroup -S storage 2>/dev/null || true
...
ln -s "$KLIPPER_PATH/config/sample-pwm-tool.cfg" "$CONFIG_PATH" || true
ln -s "$KLIPPER_PATH/config/sample-macros.cfg" "$CONFIG_PATH" || true
...
make -C "$KLIPPER_PATH" olddefconfig
